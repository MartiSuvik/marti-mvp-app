...
CREATE TABLE public.job_payments (
  ...
  CONSTRAINT job_payments_milestone_id_fkey FOREIGN KEY (milestone_id) REFERENCES public.milestones(id)
);
...
CREATE TABLE public.jobs (
  ...
  status text NOT NULL DEFAULT 'draft'::text CHECK (status = ANY (ARRAY['draft'::text, 'pending'::text, 'declined'::text, 'in_progress'::text, 'review'::text, 'revision'::text, 'approved'::text, 'paid_out'::text, 'cancelled'::text])),
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  proposal_id uuid,
